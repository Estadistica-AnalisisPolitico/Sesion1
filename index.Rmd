...
```{r}
meanOfVar=mean(estadoLocales$LOCALES_PUB_BUENESTADO_2018,na.rm = TRUE)
sdOfVar=sd(estadoLocales$LOCALES_PUB_BUENESTADO_2018,na.rm = TRUE)
...
```{r}
ggplot(estadoLocales, aes(sample=LOCALES_PUB_BUENESTADO_2018)) +
  stat_qq() + 
  stat_qq_line()
```
```{r}
...
```
```{r}
scatter = base + geom_text(aes(label=DEPARTAMENTO),size=3)
scatter
```
...
spearmanf1=cor.test(f1,data=EstPea,method='spearman')[c('estimate','p.value')]
spearmanf1
```
