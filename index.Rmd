...
Tomemos lo necesario de la tabla anterior:

```{r}
data_histoLocales[,c("xmin","xmax","count")]
```
Una gráfica clave para las variables numéricas es el **boxplot**:
...
```
Este resumen incluye medidas de centralidad y posición, pero no de dispersión, ni de kurtosis, ni de asimetría. Para ello debemos usar la función _Desc_ del paquete _DescTools_:

```{r}
library(DescTools)
moreStatsLocales=DescTools::Desc(estadoLocales$LOCALES_PUB_BUENESTADO_2018)
moreStatsLocales
```
...
```
Aqui eliminamos comas y el resultado lo volveremos numérico:
...
Creemos aquí una variable categórica (_factor_):
...
```{r, warning=FALSE, message=FALSE, echo=TRUE}
base=ggplot(data=EstPeaPob, aes(x=populoso, y=LOCALES_PUB_BUENESTADO_2018))
...
```{r}
ggpubr::ggerrorplot(data=EstPeaPob, x='populoso', y='LOCALES_PUB_BUENESTADO_2018')
```
...
library(ggpubr)
ggqqplot(data=EstPeaPob,x="LOCALES_PUB_BUENESTADO_2018") + facet_grid(~ populoso)
```


Como ello no es fácil de discernir visualmente, tenemos por costumbre calcular algún coeficiente, como el _Shapiro-Wilk_:


```{r, warning=FALSE, message=FALSE, echo=TRUE}


f4=formula(LOCALES_PUB_BUENESTADO_2018~populoso)
...
La regresión devuelve un modelo, es decir una ecuación, que recoge cómo una o más variables explicarías a otra. Para nuestro caso la variable dependiente es el estado de los locales: 

```{r, warning=FALSE, message=FALSE, echo=TRUE}

modelo1=formula(LOCALES_PUB_BUENESTADO_2018~peaOcupada + populoso)
```
...
modelo2=formula(LOCALES_PUB_BUENESTADO_2018~peaOcu_pct + populoso)
